...
| `functional_eval.kind` | no | str | `None` (≈ `none`) | `tinyvad_sim` opts into the TinyVAD behavioral-sim fidelity (F1) and the speedup/accuracy reward; any other value — including omitting `functional_eval` — runs the generic PPA reward and skips F1. This one field is the entire switch (`DesignSpec.is_tinyvad()`). |
